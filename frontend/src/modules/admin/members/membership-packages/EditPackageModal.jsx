...
import { X, Info, Plus, Clock } from 'lucide-react';

const EditPackageModal = ({ isOpen, onClose, isDarkMode, packageData }) => {
    ...
    const [timings, setTimings] = useState({
        monday: { inTime: '06:00', outTime: '22:00' },
        tuesday: { inTime: '06:00', outTime: '22:00' },
        wednesday: { inTime: '06:00', outTime: '22:00' },
        thursday: { inTime: '06:06', outTime: '22:00' },
        friday: { inTime: '06:00', outTime: '22:00' },
        saturday: { inTime: '06:00', outTime: '22:00' },
        sunday: { inTime: '06:00', outTime: '22:00' }
    });

    if (!isOpen) return null;

    const handleSessionDayChange = (day) => {
        if (day === 'allDays') {
            const newValue = !sessionDays.allDays;
            setSessionDays({
                allDays: newValue,
                mon: newValue,
                tue: newValue,
                    ...
                    <div className="space-y-5">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <select defaultValue="general" className={`w-full px-4 py-2.5 border rounded-lg text-[14px] outline-none appearance-none cursor-pointer ${isDarkMode ? 'bg-[#1a1a1a] border-white/10 text-white' : 'bg-white border-gray-300 text-gray-700'}`}>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <select defaultValue="regular" className={`w-full px-4 py-2.5 border rounded-lg text-[14px] outline-none appearance-none cursor-pointer ${isDarkMode ? 'bg-[#1a1a1a] border-white/10 text-white' : 'bg-white border-gray-300 text-gray-700'}`}>
                                    <option value="">Select</option>
                    ...
                    <div className="pt-6 border-t border-gray-100 dark:border-white/5 space-y-6">
                        ...
                        <div className="grid grid-cols-1 gap-5">
                            <div>
                                ...
                                />
                                <span className="text-[11px] text-[#f97316] mt-2 block font-medium">Example : 20</span>
                            </div>
                    ...
                    <div className="space-y-4">
                                ...
                                {[
                                    ...
                                    <div key={day.key} className="flex items-center gap-2">
                                        <input
                                            ...
                                            id={day.key}
                                            checked={sessionDays[day.key]}
                                            onChange={() => handleSessionDayChange(day.key)}
                                        />
                                        <label htmlFor={day.key} className={`text-[14px] font-bold ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{day.label}</label>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 pt-4">
                            {[
                                ...
                                <div key={day.key}>
                                    <h4 className={`text-[14px] font-bold mb-3 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{day.label}</h4>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className={`block text-[13px] font-bold mb-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>In Time</label>
                                            <div className="relative">
                                                <input
                                                    type="time"
                                                    value={timings[day.key].inTime}
                                                    onChange={(e) => setTimings({
                                                        ...timings,
                                                        [day.key]: { ...timings[day.key], inTime: e.target.value }
                                                    })}
                                                    className={`w-full px-4 py-2.5 border rounded-lg text-[14px] outline-none ${isDarkMode ? 'bg-[#1a1a1a] border-white/10 text-white' : 'bg-white border-gray-300'}`}
                                                />
                                        ...
                                        <div>
                                            <label className={`block text-[13px] font-bold mb-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Out Time</label>
                                            <div className="relative">
                                                <input
                                                    type="time"
                                                    value={timings[day.key].outTime}
                                                    onChange={(e) => setTimings({
                                                        ...timings,
                                                        [day.key]: { ...timings[day.key], outTime: e.target.value }
                                                    })}
                                                    className={`w-full px-4 py-2.5 border rounded-lg text-[14px] outline-none ${isDarkMode ? 'bg-[#1a1a1a] border-white/10 text-white' : 'bg-white border-gray-300'}`}
                                                />
                                                <Clock size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />

  ...
  return (
    <div className={`space-y-6 transition-none ${isDarkMode ? 'text-white' : 'text-black'}`}>
      ...
      <div className="flex justify-between items-center transition-none">
        <h1 className="text-[28px] font-black tracking-tight">Members Workout Card</h1>
      ...
      <p className="text-[13px] font-bold text-gray-600 dark:text-gray-400 pt-2">All Workout Plan(s) (20)</p>
      ...
      <div className="space-y-4 transition-none">
        {workoutPlans.map((plan, idx) => (
          <div key={idx} className={`p-4 rounded-xl border flex items-center justify-between transition-none ${isDarkMode ? 'bg-[#1a1a1a] border-white/5' : 'bg-[#f8f9fa] border-gray-100'}`}>
            <span className={`text-[16px] font-black tracking-tight ${isDarkMode ? 'text-white' : 'text-[#333]'}`}>{plan.name}</span>

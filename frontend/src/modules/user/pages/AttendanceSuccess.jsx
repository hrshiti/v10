...
const AttendanceSuccess = () => {
    ...
    const recentHistory = [
        { date: 'Feb 01', status: 'Present', color: 'text-emerald-400' },
        { date: 'Jan 31', status: 'Present', color: 'text-emerald-400' },
        { date: 'Jan 30', status: 'Absent', color: 'text-red-400' },
    ];

    return (
        <div className="min-h-screen bg-[#0f172a] font-sans flex flex-col relative overflow-hidden text-white">

            {/* Background Glow */}
            <div className="absolute top-0 center w-full h-[50vh] bg-emerald-500/10 blur-[100px] rounded-full pointer-events-none"></div>

            <div className="flex-1 flex flex-col max-w-md mx-auto w-full p-6 relative z-10">
                ...
                <div className="grid grid-cols-2 gap-4 mt-8">
                    ...
                    >
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Active Now</span>
                        </div>
                        <p className="text-2xl font-black text-white">153</p>
                    </motion.div>
                ...
                >
                    <div className="bg-[#0f172a] rounded-[1.4rem] border border-slate-700/50 p-5 relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4 border-b border-slate-700/50 pb-5 mb-5">
                            <div className="w-14 h-14 rounded-full p-0.5 bg-gradient-to-br from-emerald-500 to-blue-500">
                                <img
                            ...
                            <div>
                                <h3 className="font-bold text-lg text-white">Jordan Eagle</h3>
                                <p className="text-xs text-emerald-400 font-bold bg-emerald-500/10 px-2 py-0.5 rounded-full inline-block mt-1">✓ Checked In</p>
                            </div>
                        </div>
                    ...
                    <div className="bg-slate-800/50 rounded-2xl border border-slate-700/50 divide-y divide-slate-700/50">
                        {recentHistory.map((item, idx) => (
                            <div key={idx} className="flex items-center justify-between p-4">
                                ...
                                <span className={`text-xs font-bold ${item.color} bg-white/5 px-2 py-1 rounded-md`}>
                                    {item.status}
                                </span>
                            </div>
                        ))}
